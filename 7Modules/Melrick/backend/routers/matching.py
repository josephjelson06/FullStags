...
async def run_order_matching(
    ...
    try:
        ...
        return await match_full_order(session, order_id, simulate=False, changed_by_user_id=changed_by)
...
async def simulate_matching(
    ...
    if request.item_ids:
        ...
        return results
    raise HTTPException(status_code=400, detail="Provide order_id, order_item_id, or item_ids")
...
async def get_matching_logs(
    ...
    result = await session.execute(
        select(MatchingLog).where(MatchingLog.order_item_id == order_item_id).order_by(MatchingLog.rank)
    )
