...
class SingleDeliveryRequest(BaseModel):
    order_assignment_id: int = Field(..., gt=0, description="Confirmed order assignment ID")

    model_config = {
        "json_schema_extra": {
            "example": {"order_assignment_id": 42}
        }
    }
...
async def create_batch(
    ...
    try:
        ...
        total_naive = sum(float(delivery.get("naive_distance_km") or 0.0) for delivery in deliveries)
        total_optimized = sum(float(delivery.get("optimized_distance_km") or 0.0) for delivery in deliveries)
        total_savings = max(0.0, total_naive - total_optimized)
        total_savings_percent = (total_savings / total_naive * 100.0) if total_naive > 0 else 0.0
...
async def list_deliveries(
    ...
    return await list_deliveries_for_user(session=session, user=user)
...
async def get_delivery(
    ...
    try:
        return await get_delivery_for_user(session=session, delivery_id=delivery_id, user=user)
    except ValueError as exc:
...
async def get_delivery_route(
    ...
    try:
        return await get_delivery_route_geometry(session=session, delivery_id=delivery_id, user=user)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
...
async def patch_delivery_status(
    ...
    try:
        return await update_delivery_status(
            ...
            changed_by_user_id=current_user.get("sub"),
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
