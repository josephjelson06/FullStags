...
async def _supplier_profile_for_user(session: AsyncSession, user_id: int) -> SupplierProfile:
...
def _ensure_order_access(order: OrderResponse, user: dict, supplier_id: Optional[int] = None) -> None:
    role = user.get("role")
    ...
    if role == "supplier":
        ...
        if not owns_assignment:
            raise HTTPException(status_code=403, detail="Order is not assigned to supplier")
        return
...
async def create_order_route(
    ...
    buyer = await _buyer_profile_for_user(session, int(current_user["user_id"]))
    order = await create_order(session, buyer.id, payload, changed_by=int(current_user["user_id"]))
...
async def get_order_detail(
    ...
    if current_user.get("role") == "supplier":
        supplier = await _supplier_profile_for_user(session, int(current_user["user_id"]))
...
async def patch_order_status(
    ...
    if new_status == "CANCELLED":
        ...
        order = await transition_order_status(session, order_id, new_status, current_user)
...
async def confirm_assignment_route(
    ...
    buyer = detailed.order_item.order.buyer if detailed.order_item and detailed.order_item.order else None
