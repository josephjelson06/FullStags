...
async def get_my_supplier_profile(
    ...
    result = await session.execute(
        select(SupplierProfile).where(SupplierProfile.user_id == current_user["user_id"])
    )
...
async def get_supplier_summary(
    ...
    stats = await session.execute(
        select(
            ...
            func.coalesce(func.sum(PartsCatalog.unit_price * PartsCatalog.quantity_in_stock), 0),
...
async def get_supplier_catalog(
    ...
    total_result = await session.execute(
        select(func.count(PartsCatalog.id)).where(PartsCatalog.supplier_id == supplier_id)
    )
