...
async def seed_users_and_profiles() -> dict[str, User]:
    ...
    async with AsyncSessionLocal() as session:
        admin = await _ensure_user(session, email="admin@sparehub.in", role="admin", password="admin123")
        ...
        for buyer in BUYERS:
            ...
            profile_result = await session.execute(select(BuyerProfile).where(BuyerProfile.user_id == user.id))
        ...
        for supplier in SUPPLIERS:
            ...
            profile_result = await session.execute(select(SupplierProfile).where(SupplierProfile.user_id == user.id))
            if profile_result.scalar_one_or_none() is None:
                session.add(
                    SupplierProfile(
                        user_id=user.id,
    ...
    async with AsyncSessionLocal() as session:
        for category_name, subcategory in CATEGORIES:
            result = await session.execute(
                select(PartCategory).where(PartCategory.name == category_name, PartCategory.subcategory == subcategory)
            )
        ...
        for supplier in suppliers:
            user_result = await session.execute(select(User).where(User.id == supplier.user_id))
        ...
        for record in catalog_seed:
            supplier_email, category_key, part_name, part_number, brand, unit_price, qty, min_qty, lead_hours = record
    ...
    async with AsyncSessionLocal() as session:
        ...
        order_templates = [
            (
                ...
                "urgent",
                [
                    (category_lookup["Bearings:Deep Groove Ball Bearings"], "SKF-6205", "Drive shaft bearing", 20),
                    (category_lookup["Fasteners:Industrial Bolts"], "HTB-M12-75", "Mounting bolts", 200),
                ],
            ),
            (
                ...
                "critical",
                [
                    (category_lookup["Hydraulics:Hydraulic Pumps"], "A10VSO-45DR", "Press line hydraulic pump", 2),
                ],
            ),
            (
                ...
                "standard",
                [
                    (category_lookup["Power Transmission:V-Belts"], "VB-B72", "Conveyor belt replacement", 12),
                    (category_lookup["Bearings:Deep Groove Ball Bearings"], "NSK-6307", "CNC spindle bearing", 8),
                ],
            ),
        ]
        ...
        for factory_name, urgency, items in order_templates:
            ...
            existing_order_result = await session.execute(
                select(Order).where(Order.buyer_id == buyer_profile.id).order_by(Order.id.desc())
            )
            ...
            order = Order(
                ...
                status="PLACED",
                urgency=urgency,
                required_delivery_date=(datetime.now(timezone.utc) + timedelta(days=3)).replace(tzinfo=None),
            )
            session.add(order)
            await session.flush()
...
async def seed_deliveries_and_routes():
    async with AsyncSessionLocal() as session:
        assignment_result = await session.execute(
            select(OrderAssignment).where(OrderAssignment.status.in_(["PROPOSED", "ACCEPTED"])).order_by(OrderAssignment.id.asc())
        )
        ...
        for assignment in assignments:
            ...
            await session.commit()
            await create_single_delivery(assignment.id)
            seeded += 1
            if seeded >= 3:
                break
...
async def seed_sample_notifications():
    async with AsyncSessionLocal() as session:
        buyer_profile_result = await session.execute(select(BuyerProfile).order_by(BuyerProfile.id.asc()))
        buyer_profile = buyer_profile_result.scalars().first()
        supplier_profile_result = await session.execute(select(SupplierProfile).order_by(SupplierProfile.id.asc()))
