...
export function LiveTracking() {
  ...
  if (loading) return <div className="text-center py-12 text-gray-500">Loading tracking info...</div>;
  if (!order) return <div className="text-center py-12 text-red-500">Order not found.</div>;

  return (
    <div className="space-y-6">
      <button onClick={() => navigate('/buyer/orders')} className="text-sm text-blue-600 hover:underline">← Back to Orders</button>
      ...
      {!route ? (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="text-gray-500">No delivery route assigned yet.</p>
        ...
        <>
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <div className="text-xs text-gray-500 uppercase">Delivery Status</div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
              <div className="text-xs text-gray-500 uppercase">Est. Duration</div>
              ...
              {(route.stops ?? []).sort((a, b) => a.sequence_order - b.sequence_order).map((stop, i, arr) => (
                <div key={stop.id} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`h-4 w-4 rounded-full border-2 ${stop.status === 'COMPLETED' ? 'bg-green-500 border-green-500' : stop.status === 'IN_PROGRESS' ? 'bg-blue-500 border-blue-500' : 'bg-white border-gray-300'}`} />
