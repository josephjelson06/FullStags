    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          ...
          <div className={`text-2xl font-bold ${supplier.reliability_score >= 0.8 ? 'text-green-600' : 'text-yellow-600'}`}>{(supplier.reliability_score * 100).toFixed(0)}%</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <div className="text-xs text-gray-500 uppercase">Radius</div>
          <div className="text-2xl font-bold">{supplier.service_radius_km} km</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <div className="text-xs text-gray-500 uppercase">Parts</div>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
              {catalog.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" onClick={() => navigate(`/buyer/parts/${item.id}`)}>
