...
const TEMPLATES = EVENT_TYPES.map(type => ({
  type,
  title_template: `{{event_type}} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â {{entity_type}} #{{entity_id}}`,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700" onClick={save}>
          {saved ? 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Saved' : 'Save Changes'}
        </button>
      </div>

      <div className="space-y-3">
        {templates.map((t, i) => (
          <div key={t.type} className={`rounded-lg border p-4 transition-colors ${t.enabled ? 'surface-card' : 'border-gray-100 bg-gray-50 opacity-60'}`}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => toggle(i)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${t.enabled ? 'bg-blue-600' : 'bg-gray-300'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-transparent transition-transform ${t.enabled ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
                <span className="font-medium text-sm">{t.type.replace(/_/g, ' ')}</span>
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full ${t.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
