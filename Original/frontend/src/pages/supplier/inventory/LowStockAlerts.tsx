    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-400">Threshold Ã¢â€°Â¤</label>
          <input type="number" min="1" value={threshold} onChange={e => setThreshold(parseInt(e.target.value) || 10)} className="w-20 rounded  px-2 py-1 text-sm" />
        </div>
      </div>

      {items.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          <div className="text-4xl mb-2">Ã¢Å“â€¦</div>
          ...
          {items.map(item => (
            <div key={item.id} className="flex items-center justify-between rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/30">
              <div>
                ...
                <div className="text-sm text-gray-400">#{item.part_number} Ã‚Â· Ã¢â€šÂ¹{item.unit_price.toFixed(2)}</div>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <div className={`text-3xl font-bold ${item.quantity_in_stock === 0 ? 'text-red-700' : 'text-orange-600'}`}>{item.quantity_in_stock}</div>
